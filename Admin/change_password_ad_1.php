...
session_start();
if (!isset($_SESSION['admin_user_check_1'])) {
...
if(isset($_POST['sub'])){
  	...
  	$con = mysqli_connect('localhost','root','','farm_login');
	if ($input == $input1){
		if (mysqli_query($con,"update admin_users set ad_passsword = '$input' where ad_user_id = '$reg_id_no'")){
			echo "<script>alert('Password Update Successful!')</script>";
			echo("<script>window.location = 'ad_user_home.php';</script>");	
		}else{
